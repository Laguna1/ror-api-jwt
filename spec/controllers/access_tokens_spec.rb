...
RSpec.describe AccessTokensController do
  describe 'POST /create' do
    let(:params) do
      {
        data: {
          attributes: { username: 'oksana', password: 'password' }
        }
      }
    end
    ...
    context 'when invalid username provided' do
      let(:user) { create :user, username: 'notvalid', password: 'password' }
    ...
    context 'when invalid password provided' do
      let(:user) { create :user, username: 'oksana', password: 'invalid' }
      subject { post :create, params: params }
    ...
    context 'when valid data provided' do
      let(:user) { create :user, username: 'oksana', password: 'password' }
      subject { post :create, params: params }
